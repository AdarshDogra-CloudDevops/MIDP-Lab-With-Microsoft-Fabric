...
4. Next, set up an HTTP connection to import the sample World Wide Importers data into the Lakehouse. In the **Choose data sources** list, search for **Http (1)**, and select **Http (2)** from the New sources.
...
6. In the next step, enable the **Binary copy (1)** and choose **ZipDeflate (.zip) (2)** as the **Compression type** since the source is a .zip file. Keep the other fields at their default values and click **Next (3)**.

   ![add-pipeline-copy-data](../media/10/dp(6).png)

7. In the **Choose data destination** tab, search for **Lakehouse (1)** and select **wwilakehouse (2)**.

   ![add-pipeline-copy-data](../media/10/dp(7).png)

8. In the **Connect to data destination** tab, select **Browse (1)** to choose the root folder, confirm by clicking **OK (2)**, and then click **Next (3)** to continue.

   ![add-pipeline-copy-data](../media/10/dp(8).png)

9. In the **Review + save** tab, the copy summary should be set as **Binary to Binary**. Click **Save + Run**. You can schedule pipelines to refresh data periodically. In this tutorial, we only run the pipeline once. The data copy process takes approximately 10-15 minutes to complete.
