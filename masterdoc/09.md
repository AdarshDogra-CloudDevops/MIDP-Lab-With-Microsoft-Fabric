# Exercise 2: Build a lakehouse

### Estimated Duration: 120 Minutes

## Overview

In this hands-on lab, you'll go through the end-to-end process of building a modern data platform using Microsoft Fabric. The goal is to create a centralized lakehouse that combines storage, data ingestion, analytics, and reporting, demonstrating how different Fabric components work together.

You’ll start by activating SharePoint Online to enable required cloud services. Then, you’ll set up a dedicated workspace in Microsoft Fabric and create a lakehouse to store and manage structured and unstructured data. You'll ingest sample data into the lakehouse using Dataflow Gen2 and prepare it in Delta table format. Once the data is ready, you’ll switch to Power BI to build a report that visualizes the insights. You’ll configure the semantic model, explore the dataset, and auto-generate a report, all from within the same workspace.

By the end of this lab, you’ll have a full understanding of how to use Microsoft Fabric to build scalable data workflows that include data engineering, transformation, and reporting, all in one integrated environment.

## Lab objectives

- Task 1: Activate SharePoint Online
- Task 2: Create a Workspace
- Task 3: Create a lakehouse
- Task 4: Ingest sample data
- Task 5: Build a report

## Task 1: Activate SharePoint Online

**Objective:** Activate SharePoint Online by signing into the Office homepage.

1. To activate SharePoint Online, copy the **Office homepage link** and open this link inside the VM in a new tab.

   ```
   https://office.com
   ```

   >**Note:** If it asks you to sign in, enter the required credentials and sign in to the Office portal.

      ![01s](../media/01s(1).png)

1. You have now successfully signed into **Office homepage** and activated **SharePoint Online**.

   ![03s](../media/office-365(1).png)

   >**Note:** This will be required for Task 3 in the lab.

1. You can now close this tab and proceed to the next task.

## Task 2: Create a workspace

**Objective:** You will create a Fabric workspace. The workspace contains all the items needed for this lakehouse tutorial, which includes lakehouse, dataflows, Data Factory pipelines, notebooks, Power BI datasets, and reports.

1. Navigate to **Workspaces (1)** from the left pane, and click on **+ New workspace (2)** to continue.

   ![New Workspace](../media/09/ws(1).png) 

2. Fill out the **Create a workspace** form with the following details, and then click on **Advance (3)**:

   - **Name:** Enter **Fabric Lakehouse Tutorial** **<inject key="DeploymentID" enableCopy="false" />** **(1)**, and any extra characters to make the name unique.

   - **Description:** Enter **Lakehouse Demo (2)**

      ![name-and-desc-of-workspc](../media/09/ws(2).png)

4. Under **License mode**, select **Trial (4)** and click on **Apply (5)** to create and open the workspace.

   ![advanced-and-apply](../media/09/ws(3).png)

    > **Note:** If a task flows preview feature notification appears, click **Got it** to proceed. 

      ![](../media/09/ws(4).png)

5. A new workspace titled **Fabric Lakehouse Tutorial** **<inject key="DeploymentID" enableCopy="false" />** will be created. This workspace will serve as a designated area where you can organize and manage your project components, data, and reports, providing a centralized hub for all related activities and resources.

   > **Congratulations** on completing the task! Now, it's time to validate it. Here are the steps:
   > - If you receive a success message, you can proceed to the next task.
   > - If not, carefully read the error message and retry the step, following the instructions in the lab guide. 
   > - If you need any assistance, please contact us at cloudlabs-support@spektrasystems.com. We are available 24/7 to help you out.
 
   <validation step="e075f8d4-19b3-4992-9f58-df7a6b94d338" />


## Task 2: Create a lakehouse

**Objective:** Create a lakehouse within Microsoft Fabric to manage and analyze data.

1. From the newly created Power BI workspace **Fabric Lakehouse Tutorial <inject key="DeploymentID" enableCopy="false"/> (1)**, click on **+ New item (1)**, in the search bar, type **Lakehouse (2)**, and from the results, select **Lakehouse (3)** to proceed.

   ![](../media/09/lh(1).png)


1. In the New Lakehouse dialog box, enter **wwilakehouse (1)** in the Name field, then click **Create (2)** to create and open the lakehouse.

   ![](../media/221124(3).png)

   > **Congratulations** on completing the task! Now, it's time to validate it. Here are the steps:
   > - If you receive a success message, you can proceed to the next task.
   > - If not, carefully read the error message and retry the step, following the instructions in the lab guide. 
   > - If you need any assistance, please contact us at cloudlabs-support@spektrasystems.com. We are available 24/7 to help you out.
 
   <validation step="98c90546-5680-4978-b3ae-eb32a2422fdc" />

## Task 3: Ingest sample data

**Objective:** Load sample data into the lakehouse for processing and analysis.

1. In the **Lakehouse Explorer**, you see options to Get data in your lakehouse. Select **New Dataflow Gen2**.

   ![select-dataflow-gen2](../media/09/lh(2).png)

1. In the **New Dataflow Gen2** dialogue box, provide the name as **Dataflow 1 (1)** and select **Create (2)** to proceed.

   ![select-dataflow-gen2-1](../media/newdataflow-09-02.png)

   >**Note:** Make sure to **Uncheck** the 'Enable Git integration, deployment pipelines and Public API scenarios'.

3. On the new dataflow pane, select **Import from a Text/CSV file**.

   ![select-option-to-import-csv-file](../media/09/lh(3).png)

4. On the **Connect to data source (1)** pane, select the **Upload file (Preview) (2)** radio button, and then click on **Browse (3)**.

   ![Browse](../media/09/lh(4).png)

5. In the Open window, navigate to the **C:\FabricFiles (1)** folder, select the **dimension_customer.csv (2)** file, and click on **Open (3)**.

   ![navigate-csv](../media/fabric-03.png)  

6. After the file is uploaded, select **Next**.

   ![](../media/09/lh(5).png)

   >**Note:** The **Preview file data** page might require a few minutes to load.

7. From the **Preview file data** page, preview the data and select **Create** to proceed and return to the dataflow canvas.
   
   ![](../media/09/lh(6).png)

8. It will open up a **Power Query** editor. In the **Query settings (1)** pane, verify whether **dimension_customer (2)** is reflected under the **Name** field. 

   ![verify-name-and-lakehouse](../media/09/lh(7).png)

9. Next, remove the default **Data destination** by clicking the **X** icon next to the Lakehouse under the **Data destination** section.

   ![click-x](../media/09/lh(8).png)

10. A prompt will appear asking you to confirm the removal of the data destination. Click **OK** to proceed.

      ![click-ok](../media/09/lh(9).png)
   
11. In the **Query** editor, click on **Add data destination (1)**, and select **Lakehouse (2)**.

      ![set-data-lakehouse-dest](../media/09/Query(1).png)

    >**Note:** If you are unable to find the Query option, click the drop-down arrow on the right side of the box.

      ![](../media/drop-downarrow.png)

12. In the **Connect to data destination** pane, confirm the **Connection**, and click on **Next**.

    ![](../media/221124(5).png)

13. In the **Choose destination target** pane, confirm the name of the table to be **dimension_customer (1)**. Now, expand your workspace, select **wwilakehouse (2)**, and click on **Next (3)**.

    ![data-destination-lakehouse](../media/09/lh(10).png) 

14. In the **Choose destination settings**, select **Save settings** to return to the dataflow canvas.

    ![replace-as-update-method-save-settings](../media/09/lh(11).png)

15. Select **Publish** from the bottom-right of the dataflow canvas to save your changes.

    ![publish-canvas](../media/09/lh(12).png)

16. A spinning circle next to the dataflow's name indicates publishing is in progress in the item view.

    ![dataflow-in-progress](../media/09/lh(13).png)

17. Once publishing is complete, click on the **Ellipsis(...) (1)** and select **Properties (2)**.

    ![select-properties](../media/09/lh(14).png)

18. Rename the dataflow to **Load Lakehouse Table (1)** and select **Save (2)**.

    ![](../media/09/lh(15).png)

19. Click **Refresh now** next to the dataflow name to execute it. This will load data from the source file into the lakehouse table.

    ![refresh-data-flow](../media/09/lh(16).png)

20. Once the dataflow is refreshed, select **wwilakehouse** from the **Fabric workspace**.

    ![select-lakehouse](../media/09/lh(17).png)

21. Now, you can view the **dimension_customer** delta table. Select the table to preview its data.

    ![preview-customer-data](../media/09/lh(18).png)

    >**Note:** If you are unable to view _dimension_customer_ delta table, please refresh the page.

23. You can also use the SQL analytics endpoint of the lakehouse to query the data with SQL statements. Select **SQL analytics endpoint** from the Lakehouse drop-down menu at the top right of the screen.

    ![sql-endpoint](../media/09/lh(19).png)  

24. Click on the **drop-down (1)** and select **New SQL query (2)** to write your SQL statements.

    ![new-sql-query](../media/09/lh(20).png)

25. Enter the following SQL query to group and count rows by the BuyingGroup column in the **dimension_customer** table.

    ```
    SELECT BuyingGroup, Count(*) AS Total
    FROM dimension_customer
    GROUP BY BuyingGroup
    ```

26. Click on **Run (1)** to run the query and view the **Results (2)**.

    ![view-sql-query-results](../media/09/lh(21).png)

## Task 4: Build a report

**Objective:** Create a report using Power BI based on the ingested data.

1. Select **wwilakehouse** from the **Fabric workspace**.

    ![select-lakehouse](../media/09/br(1).png)

1.  Open your lakehouse and switch to the **SQL analytics endpoint** view.

    ![sql-endpoint](../media/09/lh(19).png)

1. Previously, all the lakehouse tables and views were automatically added to the semantic model. With recent updates, for new lakehouses, you must manually add your tables to the semantic model. From the **Reporting (1)** tab, select **Manage default semantic model (2)** and select the tables that you want to add to the semantic model. In this case, select the **dimension_customer (5)** table under **dbo (3)** > **Tables (4)**, Click on **Confirm (6)**.

   ![](../media/09/22ab.png)

1. To ensure that the tables in the semantic model are always in sync, switch to the **SQL analytics endpoint**, click on **Ellipsis(...) (1)**, and select **Settings (2)**. 

   ![](../media/09/br(2).png)

1. In the left pane, select **Default Power BI Semantic Model (1)** and switch Sync the default Power BI semantic model to **On (2)**.

   ![](../media/09/br(3).png)

1. From the workspace, select the **wwilakehouse Semantic model**. 

   ![](../media/09/br(4).png)

1. From the dataset pane, you can view all the tables. You have options to create reports either from scratch, paginated reports, or let Power BI automatically create a report based on your data. For this lab, select the **drop-down (1)** for Explore this data and click on **Auto-create a report (2)**.

   ![](../media/09/br(5).png)

1. Since the table is a dimension and there are no measures in it, Power BI creates a measure for the row count, aggregates it across different columns, and creates different charts as shown in the following image. You can save this report for the future by selecting **Save** from the top ribbon.

    ![save-report](../media/09/br(6).png)

1. Enter a name for your report as **Profit Reporting (1)** and click on **Save (2)**.

    ![](../media/09/br(7).png)

1. Your report will now be stored within the same Power BI workspace, ensuring that all your data and visualizations remain organized in one central location for easy access and management.

   > **Congratulations** on completing the task! Now, it's time to validate it. Here are the steps:
   > - If you receive a success message, you can proceed to the next task.
   > - If not, carefully read the error message and retry the step, following the instructions in the lab guide. 
   > - If you need any assistance, please contact us at cloudlabs-support@spektrasystems.com. We are available 24/7 to help you out.

 <validation step="98c90546-5680-4978-b3ae-eb32a2422fdc" />

## Review:

In this exercise, you have:

- Activated SharePoint Online, ensuring that required cloud-based collaboration and storage services are available for specific tasks in the lab.

- Created a dedicated workspace in Microsoft Fabric to organize and manage all related assets, including the lakehouse, dataflows, and reports.

- Created a lakehouse in Microsoft Fabric, which serves as a centralized repository for structured and unstructured data, enabling efficient data organization and processing.

- Ingested sample data into the lakehouse, preparing the environment for further analysis and enabling data exploration using a Delta table format.

- Built and saved a report using Power BI, showcasing insights through visualizations by leveraging the ingested data and lakehouse structure.

- Utilized the seamless integration of Microsoft Fabric's components to perform end-to-end data workflows, demonstrating how data engineering, analytics, and reporting work together in a single platform.

### You have successfully completed the exercise. Click on Next >> to proceed with the next exercise.

   ![](./../media/nextpage(1).png)
