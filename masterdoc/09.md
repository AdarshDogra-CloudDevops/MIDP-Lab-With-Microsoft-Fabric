...
1. From the newly created Power BI workspace **Fabric Lakehouse Tutorial <inject key="DeploymentID" enableCopy="false"/> (1)**, click on **+ New item (2)**, scroll down to **Store Data**, and select **Lakehouse (3)**.
...
1. In the **Lakehouse Explorer**, you see options to load data into Lakehouse. Select **New Dataflow Gen2**.
   ...
   ![select-dataflow-gen2](../media/newdataflow-09.png)

1. In the **New Dataflow Gen2** window, provide the name as **Dataflow 1 (1)** and select **Create (2)** to proceed.
...
9. Now, we will be removing the pre-configured **Data destination** and configuring it manually, click on the **X (1)** next to the Lakehouse icon under **Data destination**.

   ![click-x](../media/09/11.png)

10. It will open up a prompt which will ask you to confirm to **Remove data destination**, and click on **OK (1)**.
...
13. In the **Choose destination target** pane, confirm the name of the table to be **dimension_customer (1)**. Now, expand your workspace, select **wwilakehouse (2)**, and click on **Next (3)**.
    ...
    ![data-destination-lakehouse](../media/data-destination.png) 
...
20. Once the dataflow is refreshed, select **wwilakehouse (1)** from the **Fabric workspace**.
...
24. Click on the **drop-down (1)** and select **New SQL query (2)** to write your SQL statements.
    ...
    ![new-sql-query](../media/fabric-04.png)
...
1. Select **wwilakehouse (1)** from the **Fabric workspace**.
...
1. Since the table is a dimension and there are no measures in it, Power BI creates a measure for the row count, aggregates it across different columns, and creates different charts as shown in the following image. You can save this report for the future by selecting **Save** from the top ribbon.
    ...
    ![save-report](../media/fabric-07.png)
