...
4. Under **License mode**, select **Trial (5)** and click on **Apply (5)** to create and open the workspace.
...
1. From the newly created Power BI workspace **Fabric Lakehouse Tutorial <inject key="DeploymentID" enableCopy="false"/> (1)**, click on **+ New item (1)**, in the search bar, type **Lakehouse (2)**, then from the results, select **Lakehouse (3)** to proceed.

   ![](../media/09/lh(1).png)


1. In the **New lakehouse** dialog box, enter **wwilakehouse (1)** in the **Name** field, and click on **Create (2)** to create and open the new lakehouse.
...
1. In the **New Dataflow Gen2** dialogue box, provide the name as **Dataflow 1 (1)** and select **Create (2)** to proceed.
   ...
   >**Note:** Ensure to **Uncheck** the 'Enable Git integration, deployment pipelines and Public API scenarios'.
...
7. From the **Preview file data** page, preview the data and select **Create (1)** to proceed and return to the dataflow canvas.
...
9. Now, we will be removing the pre-configured **Data destination** and configuring it manually, click on the **X** next to the Lakehouse icon under **Data destination**.

   ![click-x](../media/09/lh(8).png)

10. It will open up a prompt which will ask you to confirm to **Remove data destination**, and click on **OK**.

      ![click-ok](../media/09/lh(9).png)
   
11. Now, in the **Query** editor, click on **Add data destination (1)**, and select **Lakehouse (2)**.
...
1. To ensure that the tables in the semantic model are always in sync, switch to the **SQL analytics endpoint**, click on **Ellipsis(...)** to get the setting options. 
...
1. Select **Default Power BI semantic model (1)** and turn **On (2)** Sync the default Power BI semantic model.
...
1. From the dataset pane, you can view all the tables. You have options to create reports either from scratch, paginated reports, or let Power BI automatically create a report based on your data. For this lab, select **Explore this data (1)** and click on **Auto-create a report (2)**.
